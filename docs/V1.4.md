...
| # | 功能 | 优先级 | 预估 | 依赖 |
|---|------|--------|------|------|
| 1 | 会话 thread 视图 | P0 | 2–3d | 列表 `thread_id`、`thread_message_count` | ✅ v1.4.1 |
| 2 | 仅保存草稿 | P0 | 1d | `create-draft` API（已有） | ✅ v1.4.1 |
| 3 | 公共邮箱发信身份 | P1 | 1–2d | 多 mailbox + send 参数 Explorer 复核 | ✅ v1.4.2 |
| 4 | 快捷键 | P1 | 1d | 无 | ✅ v1.4.2 |
| 5 | 深色模式 | P2 | 1d | CSS 变量（已有 `--*` 基础） | ✅ v1.4.2 |
...
- [x] 同 thread 多封在收件箱显示为 1 行（可展开）
- [x] 展开后可选任意一封阅读
- [x] 无 thread_id 的邮件行为与现版一致
- [x] 搜索模式下仍扁平列表
...
- [x] 多邮箱账号下写信可选发件地址
- [x] 单邮箱用户 UI 无变化（无发件身份下拉）
- [ ] 收件人收到邮件显示所选公共邮箱地址（需实际公共邮箱账号验证）
...
- [x] 上述快捷键在中文输入法关闭状态下可用
- [x] 写信/搜索输入时不误触发（除 Esc）
- [x] 快捷键表可从 UI 查看
